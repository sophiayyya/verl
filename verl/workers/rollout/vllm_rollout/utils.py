...
class vLLMColocateWorkerExtension:
    ...
    def update_weights_from_ipc(
        ...
        self._pre_quantized_fp8 = pre_quantized_fp8
    ...
    def _update_weights(self, weights: list[tuple[str, torch.Tensor]], peft_config: dict, base_sync_done: bool):
        if peft_config and base_sync_done:
            ...
            if is_fp8_model(self.model_runner.vllm_config):
                pre_quantized = getattr(self, "_pre_quantized_fp8", False)
                logger.info(
                    f"FP8 model detected (async), pre_quantized={pre_quantized}: "
                    f"{self.model_runner.vllm_config.quant_config}"
                )
                loaded_params = load_quanted_weights(weights, self.model_runner, pre_quantized=pre_quantized)
